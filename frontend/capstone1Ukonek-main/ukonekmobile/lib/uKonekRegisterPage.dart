...
import '../../frontend/capstone1Ukonek-main/ukonekmobile/lib/services/ml_kit_service.dart';
...
class _uKonekRegisterPageState extends State<uKonekRegisterPage> {
  ...
  Future<void> pickIDImage() async {
    ...
    if (pickedFile != null) {
      ...
      // On web: Skip OCR, just show confirmation
      if (kIsWeb) {
        setState(() {
          _idVerified = true; // Accept any image on web without verification
          _extractedOcrText = "[Web Platform - OCR not available. Image accepted.]";
        });
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text("✅ ID image uploaded successfully!"),
            backgroundColor: Colors.green.shade700,
            duration: const Duration(seconds: 2),
          ),
        );
      } else {
        // On mobile: Use Google ML Kit OCR
        await verifyID();
      }
    }
  }
  ...
  Future<void> verifyID() async {
    ...
    try {
      // Only use Google ML Kit on mobile (native) platforms - NOT on web
      // Dynamically import to avoid web compilation errors
      final GoogleMLKitService mlKit = GoogleMLKitService();
      final result = await mlKit.verifyIDText(
        _idImage!,
      ...
      setState(() { 
        _idVerified = result['isVerified']; 
        _nameMatched = result['nameMatched']; 
        _birthdayMatched = result['birthdayMatched']; 
        _extractedOcrText = result['extractedText']; 
        _isVerifying = false; 
      });
      ...
      if (result['isVerified']) { 
        message = "✅ ID verified — name & birthday matched!"; 
        bgColor = Colors.green.shade700; 
      } else if (result['nameMatched'] && !result['birthdayMatched'] && selectedDate != null) { 
        message = "⚠️ Name matched but birthday not found."; 
        bgColor = Colors.orange.shade700; 
      } else if (!result['nameMatched'] && result['birthdayMatched']) { 
        message = "⚠️ Birthday matched but name not found."; 
        bgColor = Colors.orange.shade700; 
      } else { 
        message = "❌ ID could not be verified. Check your details."; 
      ...
      children: [
        ...
        if (kIsWeb)
          Container(
            ...
            child: Row(children: [
              Icon(Icons.info_outlined, color: Colors.amber.shade700, size: 20),
              const SizedBox(width: 10),
              Expanded(
                child: Text(
                  "Web: Upload any ID image. Verification is done manually.",
        ...
        if (_idImage != null) ...[
          ...
          if (_isVerifying && !kIsWeb)
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: _idVerified ? Colors.green.shade50 : (kIsWeb ? Colors.blue.shade50 : Colors.red.shade50),
                border: Border.all(color: _idVerified ? Colors.green.shade200 : (kIsWeb ? Colors.blue.shade200 : Colors.red.shade200)),
                borderRadius: BorderRadius.circular(12),
              ),
              child: Row(children: [
                Icon(
                  _idVerified ? Icons.verified_rounded : (kIsWeb ? Icons.info_rounded : Icons.cancel_rounded),
                  color: _idVerified ? Colors.green : (kIsWeb ? Colors.blue : Colors.red),
                  size: 22,
                ),
                const SizedBox(width: 10),
                Expanded(child: Text(
                  kIsWeb ? "✅ Image uploaded successfully" : (_idVerified ? "ID Successfully Verified" : "Verification Failed"),
                  style: TextStyle(
                    color: _idVerified ? Colors.green.shade800 : (kIsWeb ? Colors.blue.shade800 : Colors.red.shade800),
            ...
            if (_extractedOcrText.isNotEmpty && !kIsWeb) ...[
